...
(function($, window, undefined) {
    ...
    var numRounds = 0;
    var angleMin = -1, angleMax = -1;
    const BPM_PER_ROUND = 20;
    var first = 0;
    var handleLocked = false;
    var oldAngle;

    function RoundSlider(n, t) {
        this.id = n.id, this.control = $(n), this.options = $.extend({}, this.defaults, t);
    }
    ...
    function CreateRoundSlider(n, t) {
        for (var i, r, u, f = 0; f < this.length; f++)
            ...
            } else u = new RoundSlider(i, n), u._saveInstanceOnElement(), u._saveInstanceOnID(), u._raise("beforeCreate") !== !1 ? (u._init(), u._raise("create")) : u._removeData();
        first = 0;
        numRounds = 0;
        angleMin = -1;
        console.log("CRIOU");
    ...
    $.fn[pluginName] = function(n) {
        ...
        _init: function() {
            var t, n;
            this._isBrowserSupport = this._isBrowserSupported(), this._isKO = !1, this._isAngular = !1, this.control.is("input") && (this._isInputType = !0, this._hiddenField = this.control, this.control = this.$createElement("div"), this.control.insertAfter(this._hiddenField), this.options.value = this._hiddenField.val() || this.options.value, t = this, this._checkKO() && setTimeout(function() {
                t._checkKO()
        ...
        _initialize: function() {
            ...
            (n && this.control.addClass("rs-" + n), this._isBrowserSupport) && (this._isReadOnly = !1, this._checkDataType(), this._refreshCircleShape())
        },
        _render: function() {
            if (this.container = this.$createElement("div.rs-container"), this.innerContainer = this.$createElement("div.rs-inner-container"), this.block = this.$createElement("div.rs-block rs-outer rs-border"), this.container.append(this.innerContainer.append(this.block)), this.control.addClass("rs-control").empty().append(this.container), this._setRadius(), this._isBrowserSupport) this._createLayers(), this._setProperties(), this._setValue(), this._updateTooltipPos(), this._bindControlEvents("_bind");
            else {
        ...
        _createLayers: function() {
            var i = this.options.width,
                t = this._start,
                n;
            n = this.$createElement("div.rs-path rs-transition"), this._rangeSlider || this._showRange ? (this.block1 = n.clone().addClass("rs-range-color").rsRotate(t), this.block2 = n.clone().addClass("rs-range-color").css("opacity", "0").rsRotate(t), this.block3 = n.clone().addClass("rs-path-color").rsRotate(t), this.block4 = n.addClass("rs-path-color").css({
                opacity: "1",
                "z-index": "1"
            }).rsRotate(t - 180), this.block.append(this.block1, this.block2, this.block3, this.block4).addClass("rs-split")) : this.block.append(n.addClass("rs-path-color")), this.lastBlock = this.$createElement("span.rs-block").css({
                padding: i
            }), this.innerBlock = this.$createElement("div.rs-inner rs-bg-color rs-border"), this.lastBlock.append(this.innerBlock), this.block.append(this.lastBlock), this._appendHandle(), this._appendOverlay(), this._appendHiddenField()
        },
        ...
        _appendTooltip: function() {
            this.container.children(".rs-tooltip").length === 0 && (this.tooltip = this.$createElement("span.rs-tooltip rs-tooltip-text"), this.container.append(this.tooltip), this._tooltipEditable(), this._updateTooltip());
        },
        ...
        _focusOut: function(n) {
            if (n.type == "change") {
                ...
                t[0] == "," && (t = "-" + t.slice(1).replace("-", ",")), this.options.value = t, this._analyzeModelValue(), this._validateModelValue(), this._setValue(), this.input.val(this._getTooltipValue(!0))
        ...
        _addSeperator: function(n, t) {
            var r = this.$createElement("span.rs-seperator rs-border"),
                ...
                f = this._border(),
                i = this.$createElement("span.rs-bar rs-transition " + t).append(r).rsRotate(n);
        ...
        _handleMove: function(n) {
            ...
            var u = this._getXY(n),
                ...
                t = this._getAngleValue(u, f, !0), //PEGA ANGULO E VALOR PRA COLOCAR NO SLIDER
                i, r;
        ...
        _handleKeyDown: function(n) {
            var t, r, u, i, f;
            this._isReadOnly || (t = n.keyCode, r = this.keys, t == 27 && this._handles().blur(), t >= 35 && t <= 40) && (t >= 37 && t <= 40 && this._removeAnimation(), u = this["_handle" + this._active], n.preventDefault(), t == r.UP || t == r.RIGHT ? i = this._round(this._limitValue(u.value + this.options.step)) : t == r.DOWN || t == r.LEFT ? i = this._round(this._limitValue(u.value - this._getMinusStep(u.value))) : t == 36 ? i = this._getKeyValue("Home") : t == 35 && (i = this._getKeyValue("End")), f = this._valueToAngle(i), this._changeSliderValue(i, f), this._raiseEvent("change"))
        },
        ...
        _getAngleValue: function(n, t, i) {
            var u = Math.atan2(n.y - t.y, t.x - n.x),
                r = -u / (Math.PI / 180);
            return r < this._start && (r += 360), r = this._newAngle(r), this._processStepByAngle(r)
        },
        _newAngle:function(newAngle) {
        	if (!first) {
        		first = 1;
        		numRounds = Math.trunc(BPM_PER_ROUND * this.options.value / (2*360));
        		oldAngle = (BPM_PER_ROUND * this.options.value / 2) % 360;
        	}

        	if (newAngle >= 0.0 && newAngle < 90.0 && oldAngle > 270.0) {
            	numRounds++;
            } else if (oldAngle < 90.0 && newAngle > 270.0) {
            	numRounds--;
            }

            oldAngle = newAngle;

            return newAngle + numRounds * 360;
        },
        _checkAngle: function(n, t) { // PRA GENTE NAO FAZ SENTIDO TER ISSO
            ...
            } else if (t && (u = this._handleDragDistance, this.isNumber(u) && Math.abs(f - r) > u)) return i;
            return n
        },
        _processStepByAngle: function(n) {
            var t = this._angleToValue(n);
            var obj = this._processStepByValue(t);
            console.log("n: " + n + ", obj.value: " + obj.value);

            if (n <= 0) {
            	obj.value = this.options.min;
                obj.angle = 0;
            }
            ...
            /*var angAnterior = this["_handle" + this._active].angle;
            var angNovo = obj.angle;
            
            if (!first)
            	first = 2 * this.options.min - this["_handle" + this._active].value;

            if (angNovo >= 0.0 && angNovo < 90.0 && angAnterior > 270.0) {
            	numRounds++;
            } else if (angAnterior < 90.0 && angNovo > 270.0) {
            	numRounds--;
            }

            obj.value += numRounds * BPM_PER_ROUND + first;
            ...
            if (obj.value < this.options.min) {
            	...
            	obj.value = this.options.min;
                obj.angle = angleMin;
            } else if (obj.value >= this.options.max) {
            	if (angleMax == -1)
                    angleMax = obj.angle;
              	
                obj.value = this.options.max;
                obj.angle = angleMax % 360.0;
            }
			*/
            return obj;
        },
        ...
        _getTooltipPos: function() {
            ...
            if (n == "full" || n == "pie" || n.indexOf("custom") === 0) return {
                "margin-top": -this.tooltip.outerHeight() - 10, // HEIGHT CUSTOM
        ...
        _validateStartAngle: function() {
            ...
            return n = (this.isNumber(n) ? parseFloat(n) : 0) % 360, n < 0 && (n += 360), this.options.startAngle = n, n
        },
        _validateEndAngle: function() {
            var end = this.options.endAngle;
            if (typeof end == "string" && this.isNumber(end) && (end.charAt(0) === "+" || end.charAt(0) === "-")) try {
                end = eval(this.options.startAngle + end.charAt(0) + Math.abs(parseFloat(end)))
            } catch (e) {
                console.warn(e)
            }
            return end = (this.isNumber(end) ? parseFloat(end) : 360) /*% 360*/, end <= this.options.startAngle && (end += 360), end
        },
        ...
        _checkOverlay: function(n, t) {
            ...
            i.length == 0 && (i = this.$createElement("div" + n + " rs-transition rs-bg-color"), this.container.append(i)), i.rsRotate(this._start + t)
        },
        _checkDataType: function() {
            var i = this.options,
                r, n, t, u = this._props();
            for (r in u.numberType) n = u.numberType[r], t = i[n], i[n] = this.isNumber(t) ? parseFloat(t) : this.defaults[n];
        ...
        _analyzeModelValue: function() {
            var n = this.options.value,
                ...
                f, u, r = this.isNumber,
                i;
            n instanceof Array && (n = n.toString()), i = typeof n == "string" ? n.split(",") : [n], this._rangeSlider ? u = typeof n == "string" ? i.length >= 2 ? (r(i[0]) ? i[0] : t) + "," + (r(i[1]) ? i[1] : e) : r(i[0]) ? t + "," + i[0] : t + "," + t : r(n) ? t + "," + n : t + "," + t : typeof n == "string" ? (f = i.pop(), u = r(f) ? parseFloat(f) : t) : u = r(n) ? parseFloat(n) : t, this.options.value = u
        },
        ...
        getBrowserName: function() {
            ...
            return !!window.opr && !!opr.addons || !!window.opera || t.indexOf(" OPR/") >= 0 ? n = "opera" : typeof InstallTrigger != "undefined" ? n = "firefox" : /*@cc_on!@*/ !1 && !document.documentMode ? window.StyleMedia ? n = "edge" : t.indexOf("Safari") != -1 && t.indexOf("Chrome") == -1 ? n = "safari" : (!window.chrome || !window.chrome.webstore) && t.indexOf("Chrome") == -1 || (n = "chrome") : n = "ie", n
        },
        ...
        _raise: function(n, t) {
            ...
            return t = t || {
                value: u.value
            }, t.id = this.id, t.control = this.control, t.options = u, i && (t.type = n, typeof i == "string" && (i = window[i]), $.isFunction(i) && (r = i.call(this, t), r = r === !1 ? !1 : r)), this.control.trigger($.Event(n, t)), r
        },
        ...
        _removeData: function() {
            ...
            $.removeData && $.removeData(n, pluginName), n.id && typeof window[n.id]._init == "function" && delete window[n.id]
        },
        ...
        var t = this;
        return t.css("-webkit-transform", "rotate(" + n + "deg)"), t.css("-moz-transform", "rotate(" + n + "deg)"), t.css("-ms-transform", "rotate(" + n + "deg)"), t.css("-o-transform", "rotate(" + n + "deg)"), t.css("transform", "rotate(" + n + "deg)"), t
